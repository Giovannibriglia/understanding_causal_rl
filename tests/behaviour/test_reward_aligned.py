...
def test_reward_aligned_beta_zero_near_uniform() -> None:
    pol = RewardAlignedExplorer(n_actions=4, beta=0.0)
...
def test_reward_aligned_requires_latent() -> None:
    pol = RewardAlignedExplorer(n_actions=4, beta=1.0, requires_latent=True)
    obs = torch.randn(8, 6)
    try:
...
def test_reward_aligned_continuous_action_support() -> None:
    pol = RewardAlignedExplorer(act_dim=3, beta=1.0)
